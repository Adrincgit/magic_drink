...
(async () => {
  const folder = path.resolve('context/rediseno/implementacion-v11/capturas');
 ...
 ['festival','[data-runway]',.49], ['join-060','[data-runway]',.60], ['join-0616','[data-runway]',.616], ['join-063','[data-runway]',.63], ['garden-entry','[data-runway]',.649], ['plaza','[data-runway]',.68], ['plaza-near','[data-runway]',.735], ['walk','[data-runway]',.765], ['door','[data-runway]',.8], ['interior','[data-runway]',.878], ['original','[data-runway]',1],
 ];
      for (const [label, selector, progress] of scenes) {
        if (process.env.GARDEN_ONLY && (progress < .64 || progress > .8 || size.name === 'desktop')) continue;
        ...
        if (progress >= .63 && progress < .86) await page.waitForSelector('[data-garden-world][data-renderer=webgl]', { state: 'attached', timeout: 30000 });
        if (progress >= .41 && progress < .60) await page.waitForSelector('[data-crowd-motion][data-renderer=webgl]', { state: 'attached', timeout: 30000 });
