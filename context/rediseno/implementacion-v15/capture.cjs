...
(async () => {
  ...
  for (const [width, height] of [[1440, 900], [2559, 1303], [390, 844]]) {
    ...
    await page.screenshot({ path: path.join(dir, `${width}-hero.png`) });
    await page.locator('[data-can-button]').click();
    await page.waitForTimeout(500);
    await page.screenshot({ path: path.join(dir, `${width}-magic.png`) });
    await page.waitForTimeout(800);
    await page.screenshot({ path: path.join(dir, `${width}-magic-peak.png`) });
    for (const [name, p] of [['hexy', .3132], ['festival', .49], ['door-start', .815], ['door', .84], ['door-end', .852], ['atrium', .878], ['directory', 1]]) {
      ...
      if (name === 'hexy') {
        ...
        await page.getByRole('button', { name: 'Lista de canciones', exact: true }).click();
      }
      await page.screenshot({ path: path.join(dir, `${width}-${name}.png`) });
    }
    await page.getByRole('button', { name: 'Preguntas de la plaza' }).click();
    await page.screenshot({ path: path.join(dir, `${width}-guide.png`) });
    await page.close();
  }
