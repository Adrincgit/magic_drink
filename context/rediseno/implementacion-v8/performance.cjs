...
(async () => {
  ...
  try {
    ...
    await page.evaluate(async()=>{await document.fonts.ready;await Promise.all([...document.querySelectorAll('[data-journey] img')].map(i=>i.decode()));});
    ...
    const report={environment:'Headless Google Chrome, 390x844, DPR 2, touch emulation, CPU slowdown 4x. Local server, warm decoded images. Not a physical phone or a network benchmark.',...result};
    writeFileSync('context/rediseno/implementacion-v8/performance.json',JSON.stringify(report,null,2));
