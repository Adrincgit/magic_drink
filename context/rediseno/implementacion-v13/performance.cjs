...
(async () => {
  ...
  try {
    ...
    const report={environment:'Headless Google Chrome, 390x844, DPR 2, touch emulation, CPU slowdown 4x. Local server, warm decoded images and preloaded WebGL garden/audience. Not a physical phone or a network benchmark.',...result};
    writeFileSync('context/rediseno/implementacion-v13/performance.json',JSON.stringify(report,null,2));
